...
ggplot() +
  geom_line(data = joined.data.daily %>% filter(yday <=end_day & yday >= start_day), aes(x = yday, y = temp.obs, color = "observations")) +
  geom_line(data = debiased.with.noise %>% filter(yday <=end_day & yday >= start_day), aes(x = yday, y = temp.mod, color = "downscaled + spatially debiased", group = interaction(NOAA.member,dscale.member))) + 
  scale_color_brewer(palette = "Set1") +
  ylab("Temperature [C]") +
  xlab("day of year") +
  theme(legend.position="bottom", text = element_text(size=15))
...
offset <- joined.obs.and.spline %>%
  ...
  dplyr::mutate(temp.offset = ifelse(hour == 4,interp.temp - AirTC_Avg, NA),
                ...
                ws.interp.ds = ifelse(interp.ws - max(ws.offset, na.rm = TRUE),WS_ms_Avg),
...
ggplot(data = offset %>% filter(doy <=end_day & doy >= start_day)) +
  geom_line(aes(x = doy, y = AirTC_Avg)) +
  geom_line(aes(x = doy, y = temp.interp.ds, group = NOAA.member))
