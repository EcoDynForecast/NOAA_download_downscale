...
sum.CI.1day.table = data_frame(metric = c("temp","RH","ws","sw","lw"),
                                      r2 = rep(0,5),
...
mean.CI.1day.table = data_frame(metric = c("temp","RH","ws","sw","lw"),
                                      r2 = rep(NA,5),
                                      mean.residual = rep(NA,5),
                                      CI.90 = rep(NA,5),
                                      CI.95 = rep(NA,5),
...
for (i in 0:13){
  table.i = assess_downscaling(start_time = as_datetime(START_TIME) + 24*60*60 * i,
                               ...
                               DOWNSCALE_MET = TRUE, ADD_NOISE = FALSE, PLOT = FALSE, PRINT = FALSE) # iterating one day at a time, 1-day intervals
  CI.1day.table.i = data_frame(metric = c("temp","RH","ws","sw","lw"),
                                      r2 = rep(NA,5),
                                      mean.residual = rep(NA,5),
                                      CI.90 = rep(NA,5),
                                      CI.95 = rep(NA,5),
  ...
  for(n in 1:nrow(CI.1day.table.i)){
    for(m in 2:ncol(CI.1day.table.i)){
      sum.CI.1day.table[n,m] = sum.CI.1day.table[n,m] + CI.1day.table.i[n,m]
    }
  }
  ...
  for(n in 1:nrow(sum.CI.1day.table)){
    for(m in 2:ncol(sum.CI.1day.table)){
      mean.CI.1day.table = sum.CI.1day.table/count
    }
  }
print(mean.1day.table)
print(sum.CI.1day.table)
